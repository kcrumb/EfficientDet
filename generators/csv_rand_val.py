...
def _split_data():
    ...
    global val_image_name
    global val_len
    # reset
    train_image_names = []
    # val_image_name = []
    # convert to do numpy operations
    images = np.array(image_names)
    # calculate validation set length
    # data_len = len(images)
    # val_len = math.ceil(val_fraction * data_len)
    # choose random indices
    val_idx = np.random.choice(len(images), val_len, replace=False)
    # set validation and training set
    val_image_name_queue.put(images[val_idx])
    # val_image_name = images[val_idx]
    train_image_names = np.delete(images, val_idx)
...
train_image_names = []
val_image_name = []
val_image_name_queue = queue.Queue()
...
class CSVGenerator(Generator):
    ...
    def __init__(
        ...
        # self.shuffle_groups = kwargs['shuffle_groups']
        ...
        self.val_faction = val_faction
        self.train_gen = train_gen
        # only training generator will load the csv; must therefore be loaded first
        if not train_gen:
        ...
        try:
            with _open_for_csv(csv_data_file) as file:
                ...
                if self.detect_quadrangle:
                    image_data = _read_quadrangle_annotations(csv.reader(file, delimiter=','), classes,
                                                                   self.detect_text)
                else:
        ...
        print('\nimage_names', len(image_names))

        # is doing the splitting
        global val_len
        val_len = math.ceil(self.val_faction * len(image_names))
        _split_data()
        val_image_name = val_image_name_queue.get()

        super(CSVGenerator, self).__init__(detect_text=detect_text, detect_quadrangle=detect_quadrangle, **kwargs)

    def on_epoch_end(self):
        Generator.on_epoch_end(self)
        self.epochs += 1
        if self.train_gen:
            # print('\n=== END EPOCH ===')
            _split_data()
            ...
            val_image_name = val_image_name_queue.get()
    #
        # if self.val_faction is not None:
    ...
    def size(self):
        ...
        if self.train_gen:
            global train_image_names
            return len(train_image_names)
        else:
            global val_image_name
            return len(val_image_name)
    ...
    def image_path(self, image_index):
        ...
        if self.train_gen:
            global train_image_names
            return osp.join(self.base_dir, train_image_names[image_index])
        else:
            global val_image_name
            return osp.join(self.base_dir, val_image_name[image_index])
    ...
    def load_image(self, image_index):
        ...
        # print('E:', self.epochs, 'Train:', self.train_gen, 'Index:', image_index, 'Path:', path)
    ...
    def load_annotations(self, image_index):
        ...
        if self.train_gen:
            global train_image_names
            path = train_image_names[image_index]
        else:
            global val_image_name
            path = val_image_name[image_index]
